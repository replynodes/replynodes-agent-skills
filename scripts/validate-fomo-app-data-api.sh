...
grep -Fx 'version: 1.0.8' "$root/SKILL.md" >/dev/null || fail 'frontmatter version'
...
if [[ -f "$root/skill-card.md" ]]; then
  ...
  grep -F '**Version:** `1.0.8`' "$root/skill-card.md" >/dev/null || fail 'local card version'
fi
