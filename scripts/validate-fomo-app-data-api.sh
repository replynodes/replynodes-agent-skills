...
grep -Fx 'description: Read-only FOMO App Data API for agent research and market data across Solana, meme coins trading, crypto, social trading, and on-chain activity; it provides no investing advice or transaction capabilities.' "$root/SKILL.md" >/dev/null || fail 'frontmatter description'
grep -Fx 'version: 1.0.1' "$root/SKILL.md" >/dev/null || fail 'frontmatter version'
...
if rg -n 'title:.*ReplyNodes|^# .*ReplyNodes|api\.fomoapi\.io|app\.replynodes\.com|Authorization: Bearer|upstream|pricing|investment advice' "$root" >/dev/null; then
...
if [[ -f "$root/skill-card.md" ]]; then
  ...
  grep -F '**Version:** `1.0.1`' "$root/skill-card.md" >/dev/null || fail 'local card version'
fi
